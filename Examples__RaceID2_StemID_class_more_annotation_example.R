##Load some important prerequisites for analysis. Script won't run without these!
{
  ...
  if("tsne" %in% rownames(installed.packages()) == FALSE) {biocLite("tsne")}
  library("tsne")
  ...
  if("scran" %in% rownames(installed.packages()) == FALSE) {biocLite("scran")}
  library("scran")
}
...
dist.gen <- function(x,method="euclidean", ...) {
  ...
  else if ( method %in% c("eupearson") ) sqrt(2*as.dist( 1 - cor(t(x),method="pearson",...) )) 
  else if ( method %in% c("logpearson") ) as.dist( 1 - cor(log2(t(x)),method="pearson",...) ) 
  else if ( method == "binary" )  dist( 1*(x > .1) ) 
  else if ( method == "relentr") dist(x,FUN=relentr) 
...
setMethod("clustdiffgenes",
          ...
          definition = function(object,pvalue){
            ...
            # Loop over clusters 
            for ( i in 1:max(cluster_assignemnts) ){
                
              # skip if cluster is empty 
              if ( sum(cluster_assignemnts == i) == 0 ) next
                
              # calculate mean expressions non-cluster (based normalized data)
              mean_expression =
                    if ( sum(cluster_assignemnts != i) > 1 ) {
                        apply(expression_matrix[,cluster_assignemnts != i],1,mean) 
                    } else { 
                        expression_matrix[,cluster_assignemnts != i] 
                    }
              ...
              # (note that the test statistic uses the mean count as # of trials, which is why scaling both
              # distributions matters; perhaps this overestimates the number of insignificant hits though --MW)
